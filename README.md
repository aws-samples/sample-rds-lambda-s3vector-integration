...
This sample demonstrates how to integrate Amazon Aurora PostgreSQL databases with AWS S3 Vector indexes through AWS Lambda functions. The integration provides PostgreSQL functions that use AWS Lambda to call S3 Vector service APIs, allowing you to perform vector similarity searches, retrieve vectors by ID, and list vectors directly from your Aurora PostgreSQL database using familiar SQL syntax.
...
- Learning how to combine Aurora's relational capabilities with S3 Vector's specialized vector storage
...
- **AWS Lambda Function**: Python-based function that integrates Aurora PostgreSQL requests with AWS S3 Vector service API calls
- **Amazon CloudFormation Infrastructure**: Automated deployment of S3 Vector bucket/index, IAM roles, policies, and Lambda function
- **AWS S3 Vector Service Integration**: Direct integration with S3 Vector indexes for similarity search and vector operations
- **IAM Roles**: Minimal permissions for Lambda invocation from Aurora and S3 Vector service access from Lambda
...
3. **S3 Vector API Call**: Lambda function translates the request and calls the appropriate AWS S3 Vector service API (QueryVectors, GetVectors, or ListVectors)
4. **Response Processing**: Lambda function processes the S3 Vector API response and returns structured JSON to PostgreSQL
...
The CloudFormation template creates all necessary AWS resources including the S3 Vector bucket, index, Lambda function, and IAM roles. You can deploy using either the AWS CLI or AWS Console.
...
# - AuroraClusterArn: Full ARN of your Aurora cluster (find in RDS Console > Configuration tab)
...
### 2. Associate Aurora Cluster with Lambda Role

After CloudFormation deployment completes, associate the Aurora cluster with the Lambda execution role. You can do this using either the AWS CLI or AWS Console.
...
2. **Associate Role with Aurora Cluster**:
   ...
   - Click "Databases" → Select your Aurora cluster
   - Click "Modify"
...
2. **Upload via Lambda Console**:
   ...
   - Click "Functions" → Find and click `s3vl-vector-query`
...
### 6. Register S3 Vector Index

Register the S3 Vector index created by CloudFormation for easy reference:

```sql
-- Register the index with a friendly name (get ARN from CloudFormation outputs)
SELECT s3vl.register_index(
    'demo-index',
    'arn:aws:s3vectors:us-west-2:123456789012:bucket/s3vl-bucket-xyz/index/s3vl-index-5d'
);
...
### 7. Populate S3 Vector Index with Sample Data

Use the provided sample data generator to populate your index. **Note**: S3 Vector operations are only available through AWS CLI and APIs - there is no console UI for vector management.
...
echo "Using S3 Vector Index ARN: $INDEX_ARN"

# Upload vectors to S3 Vector index
...
Once setup is complete, you can use the S3 Vector integration with familiar SQL syntax:
...
- Reviewing AWS documentation for Aurora PostgreSQL and S3 Vector best practices
...
For comprehensive production guidance, see the official AWS documentation for [Aurora PostgreSQL](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/) and [S3 Vector](https://docs.aws.amazon.com/s3vectors/).
...
2. Click "Databases" → Select your Aurora cluster
3. Click "Modify"
...
6. Find the `s3vl-aurora-lambda-role` and click "Remove"
...
# Verify S3 Vector resources are deleted (these should return empty or not found)
...
- **S3 Vector Resources**: Vector bucket and index with all stored vectors
